...
namespace dx
{
    ...
    {
        MappedGpuResource(ID3D11DeviceContext& context,
                          ID3D11Resource* resource, void* memory,
                          std::size_t size)
            : m_bytes{static_cast<std::byte*>(memory),
    ...
    template<typename T>
    TypedGpuBuffer<T> MakeImmutableGpuBuffer(ID3D11Device& device,
                                             gsl::span<T> vertices,
                                             BindFlag bindFlags)
    {
        return Internal::RawMakeD3DBuffer(
            device, vertices.data(),
            gsl::narrow<std::uint32_t>(vertices.size_bytes()), bindFlags,
            ResourceUsage::Immutable);
    }

    template<typename T = std::byte>
    TypedGpuBuffer<T>
    MakeDynamicGpuBuffer(ID3D11Device& device, std::uint32_t size,
                         BindFlag bindFlags, T* data = nullptr)
    {
        return Internal::RawMakeD3DBuffer(device, data, size, bindFlags,
                                          ResourceUsage::Dynamic);
    }

    template<typename T>
    TypedGpuBuffer<T> MakeImmutableVertexBuffer(ID3D11Device& device,
                                                gsl::span<T> vertices)
    {
    ...
    template<typename T = std::byte>
    TypedGpuBuffer<T> MakeDynamicVertexBuffer(ID3D11Device& device,
                                              std::uint32_t size)
    {
    ...
    TypedGpuBuffer<T> MakeDynamicVertexBuffer(ID3D11Device& device,
                                              gsl::span<T> data)
    {
        return MakeDynamicGpuBuffer(device, data.size_bytes(),
                                    BindFlag::VertexBuffer, data.data());
    }
    ...
    TypedGpuBuffer<T> MakeImmutableIndexBuffer(ID3D11Device& device,
                                               gsl::span<T> vertices)
    {
    ...
    template<typename T, typename = std::enable_if_t<is_index<T>::value>>
    TypedGpuBuffer<T> MakeDynamicIndexBuffer(ID3D11Device& device,
                                             gsl::span<T> vertices)
    {
    ...
    MappedGpuResource
    Map(ID3D11DeviceContext& context, ID3D11Buffer& gpuBuffer,
        ResourceMapType mapType = ResourceMapType::WriteDiscard);

    template<typename T>
    void UpdateWithDiscard(ID3D11DeviceContext& context,
                           ID3D11Buffer& gpuBuffer, gsl::span<T> vertices)
    {
    ...
    template<typename T>
    wrl::ComPtr<ID3D11Buffer>
    MakeConstantBuffer(ID3D11Device& device, T* cb = nullptr,
                       ResourceUsage usage = ResourceUsage::Dynamic)
    {
        return {Internal::RawMakeD3DBuffer(
            device, cb, gsl::narrow<std::uint32_t>(sizeof(*cb)),
            BindFlag::ConstantBuffer, usage)};
    }

    inline wrl::ComPtr<ID3D11Buffer> MakeConstantBuffer(ID3D11Device& device,
                                                        std::uint32_t size)
    {
        return {Internal::RawMakeD3DBuffer(device, nullptr, size,
                                           BindFlag::ConstantBuffer,
                                           ResourceUsage::Dynamic)};
    }

    void SetupIndexBuffer(ID3D11DeviceContext& context3D,
                          ID3D11Buffer& indexBuffer, std::uint32_t offset = 0);
} // namespace dx