...
namespace dx
{
    ...
    {
        MappedGpuResource(ID3D11DeviceContext& context, ID3D11Resource* resource,
            void* memory, std::size_t size)
    ...
    template<typename T>
    TypedGpuBuffer<T> MakeImmutableGpuBuffer(ID3D11Device& device, gsl::span<T> vertices, BindFlag bindFlags)
    {
        return Internal::RawMakeD3DBuffer(device, vertices.data(), gsl::narrow<std::uint32_t>(vertices.size_bytes()), bindFlags, ResourceUsage::Immutable);
    }
    ...
    MappedGpuResource Map(ID3D11DeviceContext& context, ID3D11Buffer& gpuBuffer, ResourceMapType mapType = ResourceMapType::WriteDiscard);

    template<typename T>
    void UpdateWithDiscard(ID3D11DeviceContext& context, ID3D11Buffer& gpuBuffer, gsl::span<T> vertices)
    {
    ...
    {
        return { Internal::RawMakeD3DBuffer(device, cb, gsl::narrow<std::uint32_t>(sizeof(*cb)), BindFlag::ConstantBuffer, usage) };
    }
    ...
    void SetupIndexBuffer(ID3D11DeviceContext& context3D, ID3D11Buffer& indexBuffer, std::uint32_t offset = 0);
}