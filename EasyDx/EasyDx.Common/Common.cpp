...
namespace dx
{
    std::string ws2s(std::wstring_view wstr)
    {
        ...
        const int sizeRequired = ::WideCharToMultiByte(CP_UTF8, {},
            wstr.data(), inputSize, nullptr, 0, {}, {});
        Ensures(sizeRequired > 0);
        std::string result;
        result.resize(static_cast<std::size_t>(sizeRequired));
        if (::WideCharToMultiByte(CP_UTF8, {}, wstr.data(),
            inputSize, result.data(), sizeRequired, {}, {}) == 0)
        {
            ThrowWin32();
        }
        return result;
    ...
    std::wstring s2ws(std::string_view str)
    {
        ...
        const int sizeRequired = ::MultiByteToWideChar(CP_UTF8, {},
            str.data(), inputSize, nullptr, 0);
        Ensures(sizeRequired > 0);
        std::wstring result;
        result.resize(static_cast<std::size_t>(sizeRequired));
        if (::MultiByteToWideChar(CP_UTF8, {}, str.data(),
            inputSize, result.data(), sizeRequired) == 0)
        {
            ThrowWin32();
        }
