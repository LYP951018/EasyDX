...
namespace dx
{
    ...
    class Events
    {
      public:
        ...
        [[nodiscard]] std::unique_ptr<IEventHandle> Add(Func func)
        {
            ...
            {
                handlers_.insert(
                    {handle, [func = std::move(func)](Args&&... args) {
                         func(exforward(args)...);
    ...
    //应该在 Args 中携带尽可能多的按键信息，以防在执行 event
    //前按键状态发生变化。
...
#define DefWindowEvent(name, arg) using name = WindowEvents<arg&>
    ...
    using WindowEventArgs =
        std::variant<KeyEventArgs, MouseEventArgs, CursorMoved, ResizeEventArgs,
