...
namespace dx
{
    void SetName(ID3D11DeviceChild& child, std::string_view name)
    {
        child.SetPrivateData(WKPDID_D3DDebugObjectName,
                             gsl::narrow<std::uint32_t>(name.size()),
                             name.data());
    }
    ...
    MakeRtFromTex2D(ID3D11Device& device3D, ID3D11Texture2D& texture2D)
    {
        ...
        // If the format is set to DXGI_FORMAT_UNKNOWN, then the format of
        // the resource that the view binds to the pipeline will be used.
