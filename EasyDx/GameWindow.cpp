...
namespace dx
{
        ...
        ::RegisterClassEx(&windowClass);
        {
            ...
            loop.ExecuteInThread([&] {
                windowHandle_ = ::CreateWindow(
                    kWindowClassName, title,
                    WS_OVERLAPPEDWINDOW & ~WS_THICKFRAME, CW_USEDEFAULT,
                    CW_USEDEFAULT, width ? width.value() : CW_USEDEFAULT,
                    height ? height.value() : CW_USEDEFAULT, nullptr, nullptr,
                    GetInstanceHandle(), static_cast<void*>(this));
                {
    ...
    {
        TryWin32(SetWindowPos(NativeHandle(), HWND_TOP, rect.LeftTopX,
                              rect.LeftTopY, rect.Width, rect.Height,
    ...
    void GameWindow::PrepareForDpiChanging(std::uint32_t dpiX,
                                           std::uint32_t dpiY,
