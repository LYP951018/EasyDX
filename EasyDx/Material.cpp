...
namespace dx
{
	void SetupBlending(ID3D11DeviceContext& context3D, const BlendSettings& blendSettings)
	{
		...
		{
			context3D.OMSetBlendState(blendSettings.BlendState.Get(), blendSettings.BlendFactor.data(),
				blendSettings.SampleMask);
		}
	}

	void SetupDepthStencilStates(ID3D11DeviceContext& context3D, const DepthStencilSettings& depthStencilSettings)
	{
