...
namespace dx
{
    ...
    using BindOp =
        decltype(dx::Bind(std::declval<ID3D11DeviceContext&>(), std::declval<Bindable&>()));
    ...
    template<typename T1, typename T2, typename... BindableT,
             typename = std::enable_if_t<std::conjunction_v<is_bindable<T1&&>, is_bindable<T2&&>,
                                                            is_bindable<BindableT&&>...>>>
