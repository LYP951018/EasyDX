...
namespace dx
{
    ...
    class Mesh : Noncopyable
    {
      public:
        ...
        gsl::span<std::add_const_t<T>> GetStream(std::uint32_t streamId) const
        {
            ...
            const auto start =
                reinterpret_cast<std::add_const_t<T>*>(bytes.data());
            const auto end = reinterpret_cast<std::add_const_t<T>*>(
        ...
        void SetAllStreams(gsl::span<Args, N>... vertices)
        {
            ...
            bool equals = std::equal(
                ...
                [](const StreamInfo& info, std::size_t size) {
                    return info.Stride == size;
                });
            Ensures(equals);
        ...
        GetFullInputElementDesces() const;
        D3D11_PRIMITIVE_TOPOLOGY GetPrimitiveTopology() const;
        gsl::span<const StreamInfo> GetStreamsInfo() const
        {
            return gsl::make_span(m_streams);
        }
        gsl::span<const VSSemantics> GetChannelMasks() const
        {
        ...
        void FlushStream(ID3D11DeviceContext& context3D,
                         std::uint32_t streamId) const;

        // unowned
        template<typename... Args>
        static std::shared_ptr<Mesh> CreateImmutable(
            ID3D11Device& device, D3D11_PRIMITIVE_TOPOLOGY topology,
            gsl::span<const ShortIndex> indices, const VSSemantics* semantics,
            std::uint32_t vertexCount,
            std::vector<D3D11_INPUT_ELEMENT_DESC> inputElementDesces,
            Args*... vertices)
        {
            const auto channelsInBytes = std::array{
                gsl::as_bytes(gsl::make_span(vertices, vertexCount))...};
            const auto strides =
                std::array{gsl::narrow<std::uint32_t>(sizeof(Args))...};
            return std::make_shared<Mesh>(CreateImmutable(
                device, channelsInBytes.size(), channelsInBytes.data(),
                strides.data(), semantics, std::move(inputElementDesces),
                indices, topology));
        }

        // template<typename... Args>
        // static std::shared_ptr<Mesh>
        //	CreateImmutable(ID3D11Device& device,
        // D3D11_PRIMITIVE_TOPOLOGY topology, 		gsl::span<const
        // ShortIndex>
        // indices, const VSSemantics* semantics, 		std::uint32_t
        // vertexCount, std::vector<Args> ... vertices)
        //{
        //	CreateImmutable()
        //}

        static Mesh CreateImmutable(
      ...
      private:
        ...
        // TODO: 这里用 vector 太浪费了，倾向于 unique_ptr<T[]> +
        // m_channelCount
    ...
    // void
    // InputElementDescsFromMesh(std::vector<D3D11_INPUT_ELEMENT_DESC>&
    // inputElementDesces, const Mesh& mesh, VSSemantics semanticesToUse);
} // namespace dx