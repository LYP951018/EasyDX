...
TessQuad MakeQuad(ID3D11Device& device, const dx::PredefinedResources& predefined)
{
    ...
    return TessQuad{
        TessRenderable {
            dx::GpuMesh{device, vertices, indices},
        ...
        DirectX::XMMatrixRotationX(DirectX::XM_PI / 4.0f) * DirectX::XMMatrixRotationZ(DirectX::XM_PIDIV4)
        //DirectX::XMMatrixRotationQuaternion(DirectX::XMQuaternionRotationAxis(DirectX::XMVectorSet(1.0f, 0.0f, 0.0f, 0.0f), -DirectX::XM_PI / 2.0f))
    };
}
    ...
    }
{
    ...
    camera.SetLookAt({ 1.0f, 1.5f, -3.5f }, {}, DirectX::XMFLOAT3{ 0.0f, 1.0f, 0.0f });
    AddCameraMovement();
}
...
void MainScene::Update(const dx::UpdateArgs& args)
{
    ...
    ps.UpdateCb(context, dx::cb::GlobalLightingInfo{
        eyePos,
        1,
        {dx::cb::Light{light_}}
    }, dx::cb::PerObjectLightingInfo{
        dx::cb::Material{ smoothness, false }
    });

    const CbPerObject perObject{
        world,
        world * view * projection
    };

    const CbPerFrame perFrame{
        eyePos
    };
